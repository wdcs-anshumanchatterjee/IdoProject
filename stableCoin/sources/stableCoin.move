module coin_address1::stableCoin {
    use std::signer;
    use std::string;
    use std::vector;
    ...
    struct Capabilities has key {
        ...
        freeze_cap:coin::FreezeCapability<StableCoin>
    } 
    ...
    public entry fun register_Account(user:&signer) {
        ...
        if(!coin::is_account_registered<StableCoin>(user_address)){
            coin::register<StableCoin>(user);
        };
    ...
    public entry fun issue(user:&signer) acquires CoinAccessData{
        assert!(!exists<Capabilities>(@coin_address1),1);
        let user_address=signer::address_of(user);
        let access=borrow_global<CoinAccessData>(@coin_address1);
    ...
    public entry fun set_admin(admin: &signer, admin_addr: address) acquires CoinAccessData{
        ...
        let access=borrow_global_mut<CoinAccessData>(@coin_address1);
    ...
    public entry fun mint(user:&signer,to:address,_value:u64) acquires CoinAccessData,CoinMetadata,Capabilities{
        assert!(exists<Capabilities>(@coin_address1),5);
        let user_address=signer::address_of(user);
        assert!(isAuthorized(user_address),6);
        assert!(is_account_registered(to),7);
        let mint_cap = &borrow_global<Capabilities>(@coin_address1).mint_cap;
        let coins = coin::mint<StableCoin>(_value, mint_cap);
        coin::deposit<StableCoin>(to, coins);
    ...
    public entry fun burn(user:&signer,to:address,_value:u64) acquires CoinAccessData,CoinMetadata,Capabilities{
        ...
        assert!(is_account_registered(to),7);
        let burn_cap = &borrow_global<Capabilities>(@coin_address1).burn_cap;
        coin::burn_from<StableCoin>(to,_value, burn_cap);
        let coin_store = borrow_global_mut<CoinMetadata>(to);
    ...
    public entry fun freezing(user:&signer,to:address,_value:u64) acquires CoinAccessData,CoinMetadata,Capabilities{
        ...
        assert!(isAuthorized(user_address),6);
        assert!(is_account_registered(to),7);
        let freeze_cap = &borrow_global<Capabilities>(@coin_address1).freeze_cap;
        coin::freeze_coin_store<StableCoin>(to, freeze_cap);
        let coin_store = borrow_global_mut<CoinMetadata>(to);
    ...
    public entry fun unFreezing(user:&signer,to:address,_value:u64) acquires CoinAccessData,CoinMetadata,Capabilities{
        ...
        assert!(isAuthorized(user_address),6);
        assert!(is_account_registered(to),7);
        let mint_cap = &borrow_global<Capabilities>(@coin_address1).freeze_cap;
        coin::unfreeze_coin_store<StableCoin>(to, mint_cap);
        let coin_store = borrow_global_mut<CoinMetadata>(to);
